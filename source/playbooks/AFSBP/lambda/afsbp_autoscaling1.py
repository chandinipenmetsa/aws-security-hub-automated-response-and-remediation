...
def lambda_handler(event, context):
    ...
    log.info(event)
    #==========================================================================
    # parse AutoScaling Group Name from Security Hub CWE

    try:
        asg_name = str(event['detail']['findings'][0]['Resources'][0]['Id']).split('/')[-1]
        ssm = boto3.client('ssm')
        response = ssm.start_automation_execution(
            # Launch SSM Doc via Automation
            DocumentName='AFSBP-Autoscaling1Remediation',
            Parameters={
                'AsgName': [ asg_name ],
