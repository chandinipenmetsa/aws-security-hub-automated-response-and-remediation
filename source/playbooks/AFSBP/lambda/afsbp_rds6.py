...
def lambda_handler(event, context):
    ...
    log.info(event)
    #==========================================================================
    # parse DB Resource ID from Security Hub CWE

    try:
        resource_id = str(event['detail']['findings'][0]['Resources'][0]['Details']['AwsRdsDbInstance']['DbiResourceId'])
        ...
        response = ssm.start_automation_execution(
            ...
            Parameters={
                ...
                'MonitoringRoleArn': [os.environ['RDS_ENHANCED_MONITORING_ROLE_ARN']],
                'AutomationAssumeRole': [os.environ['IAM_ROLE']]
            }
        )
        log.info("Start Automation Execution Response: {}".format(response))
