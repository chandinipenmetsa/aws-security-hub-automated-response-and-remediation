...
def lambda_handler(event, context):
    ...
    log.info(event)
    #==========================================================================
    # parse DB Snapshot identifier from Security Hub CWE

    try:
        db_snapshot_identifier = str(event['detail']['findings'][0]['Resources'][0]['Details']['AwsRdsDbSnapshot']['DbSnapshotIdentifier'])
        ssm = boto3.client('ssm')
        response = ssm.start_automation_execution(
            # Launch SSM Doc via Automation
            DocumentName='AFSBP-RDS1Remediation',
            Parameters={
