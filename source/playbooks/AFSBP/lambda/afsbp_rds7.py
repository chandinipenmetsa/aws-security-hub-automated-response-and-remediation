...
def lambda_handler(event, context):
    ...
    log.info(event)
    #==========================================================================
    # parse DB Cluster Identifier from Security Hub CWE

    try:
        db_cluster_identifier = str(event['detail']['findings'][0]['Resources'][0]['Details']['AwsRdsDbCluster']['DbClusterIdentifier'])
        ssm = boto3.client('ssm')
        response = ssm.start_automation_execution(
            # Launch SSM Doc via Automation
            DocumentName='AFSBP-RDS7Remediation',
            Parameters={
